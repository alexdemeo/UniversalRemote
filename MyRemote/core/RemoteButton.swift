...
struct RemoteButton : Identifiable {
    ...
    private func roku() {
        ...
        AppDelegate.instance.netAsync(url: "\(AppDelegate.settings.rokuBaseURL)\(self.commandStr)", method: "POST")
    }
    
    private func cec(method: String) {
        AppDelegate.instance.netAsync(url: "\(AppDelegate.settings.cecBaseURL)\(self.commandStr)", method: method)
    }
}
